...
//						rsInterface.actions[3] = "Sell All";
			...
			if (rsInterface.type == 4) {
				rsInterface.message = stream.readString().replaceAll("RuneScape", ClientProperties.getInstance().getGameName());
...
//		}
	...
	public static void bank(TextDrawingArea[] tda) {
		...
		addText(58064, "Bank Of " + ClientProperties.getInstance().getGameName(), tda, 1, 0xE68A00, true, true);
...
//		rsi.contentType = 205;
	...
	protected static RSInterface addInterface(int id) {
		RSInterface rsi = interfaceCache[id] = new RSInterface();
		rsi.id = id;
		rsi.parentID = id;
		rsi.width = 512;
	...
	protected static void setBounds(int ID, int X, int Y, int frame, RSInterface RSinterface) {
	...
	protected static RSInterface addTabInterface(int id) {
		RSInterface tab = interfaceCache[id] = new RSInterface();
		tab.id = id;// 250
		tab.parentID = id;// 236
		tab.type = 0;// 262
